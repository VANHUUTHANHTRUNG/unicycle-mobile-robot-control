...
t_max = 1.5*np.pi  # total simulation duration in seconds
# Set initial state
init_state = np.array([1.7, 0.01, np.pi])  # px, py, theta
...
def simulate_control():
    ...
    current_input = np.array([0., 0.])  # initial numpy array for [vx, vy, omega]
    ...
    for it in range(sim_iter):
        ...
        error_ = desired_state[:2] - robot_state[:2]
        theta_ = atan2(error_[1],error_[0])
        theta = theta_ - desired_state[2]
        alpha = theta - robot_state[2]

        alpha = ((alpha + np.pi) % (2 * np.pi)) - np.pi

        error = np.linalg.norm(error_)

        u = GAMMA*np.cos(alpha)*error
        w = K*alpha + (1/alpha)*GAMMA*np.cos(alpha)*np.sin(alpha)*(alpha + H*theta)
...
if __name__ == '__main__':
    ...
    fig2 = plt.figure(2)
    ax = plt.gca()
    ax.plot(t, input_history[:,0], label='wx')
    ax.plot(t, input_history[:,1], label='wy')
