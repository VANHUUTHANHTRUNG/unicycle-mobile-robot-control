...
rot_matrix = np.array([[np.cos(np.pi/2), -np.sin(np.pi/2), 1],
    [np.sin(np.pi/2), np.cos(np.pi/2), 0.5], 
    [0, 0, 1]])
...
def transform(arr, rot_matrix):
    rot_matrix = np.linalg.inv(rot_matrix)
    arr_ = rot_matrix @ np.array([arr[0], arr[1], 1])
    arr_[2] = arr[2] - np.pi/2
    arr = arr_ 
    return arr

def inv_transform(arr, rot_matrix):
    arr_ = rot_matrix @ np.array([arr[0], arr[1], 1])
    arr_[2] = arr[2] + np.pi/2
    arr = arr_ 
    return arr
...
def simulate_control():
    ...
    desired_state = np.array([1, 0.5, np.pi/2])  # numpy array for goal / the desired [px, py, theta]
    current_input = np.array([0., 0.])  # initial numpy array for [v, w]i

    desired_state = transform(desired_state, rot_matrix)

    robot_state = transform(robot_state, rot_matrix)
    ...
    if IS_SHOWING_2DVISUALIZATION:  # Initialize Plot
        ...
        sim_visualizer.show_goal(inv_transform(desired_state, rot_matrix))

    for it in range(sim_iter):
        # record current state at time-step t
        state_history[it] = inv_transform(robot_state, rot_matrix)
        goal_history[it] = inv_transform(desired_state, rot_matrix)
        ...
        alpha = ((alpha + np.pi) % (2 * np.pi)) - np.pi  # ensure alpha within [-pi pi]
        beta = ((beta + np.pi) % (2 * np.pi)) - np.pi  # ensure beta within [-pi pi]
        ...
        if IS_SHOWING_2DVISUALIZATION:  # Update Plot
            ...
            sim_visualizer.update_goal(inv_transform(desired_state, rot_matrix))
